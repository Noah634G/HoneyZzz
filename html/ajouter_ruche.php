        ...
        <form action="ajouter_ruche_action.php" method="POST">
            ...
            <div class="form-group">
                ...
                <select name="statut" id="statut">
                   ...
                   w <option value="inactive">Inactive</option>
                </select>
            </div>

            <div class="form-group">
                <label for="idEmplacement">Emplacement de la ruche</label>
                <select name="idEmplacement" id="idEmplacement" required>
                    <option value="">-- Sélectionnez un lieu --</option>
                    <?php
                    // A. On récupère d'abord l'ID numérique de l'apiculteur
                    ...
                    $idConnected = $dataUser['idUtilisateur'];

                    // B. On cherche ses emplacements dans la table Emplacement
                    $sqlEmpl = "SELECT idEmplacement, nomLieu, adresse FROM Emplacement WHERE idUtilisateur = $idConnected";
                    $resEmpl = mysqli_query($connexion, $sqlEmpl);

                    if (mysqli_num_rows($resEmpl) > 0) {
                        while($row = mysqli_fetch_assoc($resEmpl)) {
                            echo "<option value='".$row['idEmplacement']."'>";
                            echo htmlspecialchars($row['nomLieu']) . " (" . htmlspecialchars($row['adresse']) . ")";
                            echo "</option>";
                        }
                    } else {
                        echo "<option value=''>Aucun emplacement trouvé</option>";
                    }
                    ?>
                </select>
            </div>
